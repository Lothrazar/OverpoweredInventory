...
public class InventoryRenderer {
	private static final int topspace = 12 + Const.SQ;// space used by top half

	private static final int left = 7, pad = 4;
	...
	private static int colFromSegment(int segment) {
		...
		if (segment <= segsLeft)
			if (segment % 2 == 0)
				return 2; // starting with odd row in the top right for segment
							// 1
			else
				return 1;
		else
	...
	public static int yPosSlotsStart(int segment) {
		int row = rowFromSegment(segment);
		int topLimit = topspace + 1;// 13+Const.SQ;//DIFFERENT than topSpace -
